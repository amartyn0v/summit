...
- `npm run ingest:naturescience` — скачивает **только первый найденный** PDF с указанных страниц журнала Nature Science, извлекает из него текст и отправляет в Telegram заголовок, полученный напрямую из содержимого PDF. Скрипт записывает файл в `storage/naturescience/` и помечает ссылку в `storage/naturescience/processed.json`, чтобы повторные запуски пропускали уже обработанный документ.
