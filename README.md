...
- `npm run ingest:naturescience` — скачивает **только первый найденный** PDF с указанных страниц журнала Nature Science, извлекает из него весь текст до секции `Abstract` и отправляет этот блок (обычно заголовок + авторы) в Telegram. После этого скрипт загружает тот же PDF в OpenAI, просит сформулировать простое summary, а затем переводит его на русский язык: в канал прилетают два отдельных сообщения (оригинальный пересказ и русская версия). Локальная копия файла кладётся в `storage/naturescience/`, а ссылка заносится в `storage/naturescience/processed.json`, чтобы повторные запуски пропускали уже обработанный документ. Для повторного тестирования можно выполнить `npm run ingest:naturescience -- --reset`, чтобы очистить кэшированные файлы и список обработанных ссылок (аналогичный флаг `--clear-processed`).
