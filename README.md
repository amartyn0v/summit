...
- `npm run ingest:naturescience` — скачивает **только первый найденный** PDF с указанных страниц журнала Nature Science, извлекает из него весь текст до секции `Abstract` и отправляет этот блок (обычно заголовок + авторы) в Telegram. После этого скрипт загружает тот же PDF в OpenAI и просит сформулировать простое summary только на русском языке — в Telegram прилетает один итоговый пост с этим пересказом. Если OpenAI недоступен (например, возвращает `403 Country, region, or territory not supported`), скрипт автоматически формирует резервное краткое сообщение на русском языке с ключевыми предложениями из оригинала и пометкой о ручной проверке. Локальная копия файла кладётся в `storage/naturescience/`, а ссылка заносится в `storage/naturescience/processed.json`, чтобы повторные запуски пропускали уже обработанный документ. Для повторного тестирования можно выполнить `npm run ingest:naturescience -- --reset`, чтобы очистить кэшированные файлы и список обработанных ссылок (аналогичный флаг `--clear-processed`).
