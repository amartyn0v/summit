...
- `npm run ingest:naturescience` — скачивает PDF с указанных страниц журнала Nature Science, генерирует саммари через OpenAI и отправляет текст в Telegram. Если OpenAI недоступен, скрипт построит короткое эвристическое саммари и добавит предупреждение в сообщение. Повторные запуски пропускают уже обработанные PDF (список хранится в `storage/naturescience/processed.json`).
